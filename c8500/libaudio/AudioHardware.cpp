...
static uint32_t SND_DEVICE_CURRENT=-1;//31
static uint32_t SND_DEVICE_HANDSET=-1;//actual 0
static uint32_t SND_DEVICE_MONO_HEADSET=-1;//actual 2
static uint32_t SND_DEVICE_SPEAKER=-1;//actual 6
static uint32_t SND_DEVICE_BT=-1;//12
static uint32_t SND_DEVICE_BT_EC_OFF=-1;//28
static uint32_t SND_DEVICE_HEADSET=-1;//actual 3
static uint32_t SND_DEVICE_HEADSET_AND_SPEAKER=-1;//29
static uint32_t SND_DEVICE_IN_S_SADC_OUT_HANDSET=-1;//16
static uint32_t SND_DEVICE_IN_S_SADC_OUT_SPEAKER_PHONE=-1;//25
static uint32_t SND_DEVICE_TTY_HEADSET=-1;//actual 8
static uint32_t SND_DEVICE_TTY_HCO=-1;//10
static uint32_t SND_DEVICE_TTY_VCO=-1;//9
static uint32_t SND_DEVICE_CARKIT=-1;
static uint32_t SND_DEVICE_FM_SPEAKER=-1;//27
static uint32_t SND_DEVICE_FM_HEADSET=-1;//26
...
#endif
{
   if (get_audpp_filter() == 0) {
    ...
    if (m7xsnddriverfd >= 0) {
        int rc = ioctl(m7xsnddriverfd, SND_GET_NUM_ENDPOINTS, &mNumSndEndpoints);//mNumSndEndpoints=15
        if (rc >= 0) {
...
status_t AudioHardware::setParameters(const String8& keyValuePairs)
{
    ...
    LOGV("setParameters() %s", keyValuePairs.string());//routing=2
    ...
    if (param.get(key, value) == NO_ERROR) {
        if (value == BT_NREC_VALUE_ON) {
            ...
            LOGV("Turning noise reduction and echo cancellation off for BT headset");
        }
    }
    ...
    if (param.get(key, value) == NO_ERROR) {
        ...
        for (int i = 0; i < mNumSndEndpoints; i++) {
            if (!strcasecmp(value.string(), mSndEndpoints[i].name)) {
                ...
                LOGV("Using custom acoustic parameters for %s", value.string());
                break;
            }
        }
        if (mBluetoothId == 0) {
            LOGV("Using default acoustic parameters (%s not in acoustic database)", value.string());
            doRouting(NULL);
        }
    }
    ...
    if (param.get(key, value) == NO_ERROR) {
        if (value == "true") {
            ...
            LOGV("DualMike feature Enabled");
        } else {
            mDualMicEnabled = false;
            LOGV("DualMike feature Disabled");
        }
        doRouting(NULL);
    }
...
int check_and_set_audpp_parameters(char *buf, int size)
{
    ...
    if ((buf[0] == 'A') && ((buf[1] == '1') || (buf[1] == '2') || (buf[1] == '3'))) {
        ...
        if (!(p = strtok(NULL, seps)))
            goto token_err;
        LOGV("EQ flag = %02x.", eq_flag[device_id]);
		...
		//Add by Joey Jiao since there is parse error and this error will cause mic mute during call
		return 0;
    ...
    while (1) {
        ...
        *next_str++ = '\0';
        if (check_and_set_audpp_parameters(current_str, len)) {
            LOGV("failed to set audpp parameters, exiting.");
            munmap(read_buf, st.st_size);
            close(csvfd);
            return -1;
        }
...
static int msm72xx_enable_postproc(bool state)
{
    int fd;
    int device_id=0;
//disabled by Joey Jiao to allow below problem to execute because audpp_filter_inited=false actually
/*
    ...
    {
        device_id = 0;
        LOGV("set device to SND_DEVICE_SPEAKER device_id=0");
    }
    if(snd_device == SND_DEVICE_HANDSET)
    {
        device_id = 1;
        LOGV("set device to SND_DEVICE_HANDSET device_id=1");
    }
    if(snd_device == SND_DEVICE_HEADSET)
    {
        device_id = 2;
        LOGV("set device to SND_DEVICE_HEADSET device_id=2");
    }
    ...
    else if (adrc_filter_exists[device_id] && state)
    {
        ...
        {
            LOGV("ADRC Filter ADRC FLAG = %02x.", adrc_flag[device_id]);
            LOGV("ADRC Filter COMP THRESHOLD = %02x.", adrc_cfg[device_id].adrc_params[0]);
            LOGV("ADRC Filter COMP SLOPE = %02x.", adrc_cfg[device_id].adrc_params[1]);
            LOGV("ADRC Filter COMP RMS TIME = %02x.", adrc_cfg[device_id].adrc_params[2]);
            LOGV("ADRC Filter COMP ATTACK[0] = %02x.", adrc_cfg[device_id].adrc_params[3]);
            LOGV("ADRC Filter COMP ATTACK[1] = %02x.", adrc_cfg[device_id].adrc_params[4]);
            LOGV("ADRC Filter COMP RELEASE[0] = %02x.", adrc_cfg[device_id].adrc_params[5]);
            LOGV("ADRC Filter COMP RELEASE[1] = %02x.", adrc_cfg[device_id].adrc_params[6]);
            LOGV("ADRC Filter COMP DELAY = %02x.", adrc_cfg[device_id].adrc_params[7]);
    ...
    {
        LOGV("IIR Filter FLAG = %02x.", rx_iir_flag[device_id]);
        LOGV("IIR NUMBER OF BANDS = %02x.", iir_cfg[device_id].num_bands);
        LOGV("IIR Filter N1 = %02x.", iir_cfg[device_id].iir_params[0]);
        LOGV("IIR Filter N2 = %02x.",  iir_cfg[device_id].iir_params[1]);
        LOGV("IIR Filter N3 = %02x.",  iir_cfg[device_id].iir_params[2]);
        LOGV("IIR Filter N4 = %02x.",  iir_cfg[device_id].iir_params[3]);
        LOGV("IIR FILTER M1 = %02x.",  iir_cfg[device_id].iir_params[24]);
        LOGV("IIR FILTER M2 = %02x.", iir_cfg[device_id].iir_params[25]);
        LOGV("IIR FILTER M3 = %02x.",  iir_cfg[device_id].iir_params[26]);
        LOGV("IIR FILTER M4 = %02x.",  iir_cfg[device_id].iir_params[27]);
        LOGV("IIR FILTER M16 = %02x.",  iir_cfg[device_id].iir_params[39]);
        LOGV("IIR FILTER SF1 = %02x.",  iir_cfg[device_id].iir_params[40]);
    ...
    if(state){
        LOGV("Enabling post proc features with mask 0x%04x", post_proc_feature_mask);
        ...
        LOGV("disabling post proc features with mask 0x%04x", post_proc_feature_mask);
...
status_t AudioHardware::setVoiceVolume(float v)
{
    ...
    int vol = lrint(v * 5.0);//modified from 7.0 to 5.0 by Joey Jiao
    //LOGD("setVoiceVolume(%f)\n", v);
    LOGV("Setting in-call volume to %d (available range is 0 to 5)\n", vol);

    if ((mCurSndDevice != -1) && ((mCurSndDevice == SND_DEVICE_TTY_HEADSET) || (mCurSndDevice == SND_DEVICE_TTY_VCO)))
    {
        vol = 1;
        LOGV("For TTY device in FULL or VCO mode, the volume level is set to: %d \n", vol);
    }
...
{
    android::Mutex::Autolock lock(mLock);
    int vol = ceil(v * 5.0);//modified from 7.0 to 5.0 by Joey Jiao
    LOGV("Set master volume to %d.\n", vol);
...
/*There is fucking thing since R3 after recompile android src, 
	when inCall, another person cannot hear me, 
	so have to add below code; but not for sure if will influence power consumption*/
    //if(isInCall())
	//		mMicMute=false;
    LOGV("Inside device=%d,isInCall=%s,SND_DEVICE_SPEAKER=%d",device,isInCall()?"true":"false",SND_DEVICE_SPEAKER);

#ifdef HAVE_FM_RADIO
    if(mFmRadioEnabled && (device == SND_DEVICE_FM_HEADSET)) {
      mute = 0;
      LOGV("unmute for radio");
    }
#endif
...
status_t AudioHardware::doRouting(AudioStreamInMSM72xx *input)
{
    ...
    if (input != NULL) {
        ...
        LOGV("do input routing device %x\n", inputDevice);
        ...
        if (inputDevice != 0) {
            if (inputDevice & AudioSystem::DEVICE_IN_BLUETOOTH_SCO_HEADSET) {
                LOGV("Routing audio to Bluetooth PCM\n");
                new_snd_device = SND_DEVICE_BT;
            } else if (inputDevice & AudioSystem::DEVICE_IN_WIRED_HEADSET) {
#ifdef HAVE_FM_RADIO
				if (mFmRadioEnabled) {
						LOGV("Routing audio to FM Headset\n");
						new_snd_device = SND_DEVICE_FM_HEADSET;
				} else 
#endif
				{
						LOGV("Routing audio to Wired Headset\n");
...
#ifdef HAVE_FM_RADIO
                    if (mFmRadioEnabled) {
                        LOGV("Routing audio to FM Speakerphone\n");
                        new_snd_device = SND_DEVICE_FM_SPEAKER;
                    } else 
#endif
                    {
                        LOGV("Routing audio to Speakerphone\n");
                        new_snd_device = SND_DEVICE_SPEAKER;
                    }
                    new_post_proc_feature_mask = (ADRC_ENABLE | EQ_ENABLE | RX_IIR_ENABLE | MBADRC_ENABLE);
                } else {
                    LOGV("Routing audio to Handset\n");
...
#ifdef HAVE_FM_RADIO
    LOGV("doRouting CALLED, outputDevices = 0x%x, mFmRadioEnabled = %d",outputDevices, mFmRadioEnabled);
#endif
    ...
    if (new_snd_device == -1) {
        ...
        if ((mTtyMode != TTY_OFF) && (mMode == AudioSystem::MODE_IN_CALL) &&
            ...
            LOGV("Routing audio to Wired Headset and Speaker\n");
            new_snd_device = SND_DEVICE_HEADSET_AND_SPEAKER;
            new_post_proc_feature_mask = (ADRC_ENABLE | EQ_ENABLE | RX_IIR_ENABLE | MBADRC_ENABLE);
        } else if (outputDevices & AudioSystem::DEVICE_OUT_WIRED_HEADPHONE) {
            if (outputDevices & AudioSystem::DEVICE_OUT_SPEAKER) {
                LOGV("Routing audio to No microphone Wired Headset and Speaker (%d,%x)\n", mMode, outputDevices);
                new_snd_device = SND_DEVICE_HEADSET_AND_SPEAKER;
                new_post_proc_feature_mask = (ADRC_ENABLE | EQ_ENABLE | RX_IIR_ENABLE | MBADRC_ENABLE);
            } else {
#ifdef HAVE_FM_RADIO
                if (mFmRadioEnabled) {
                    LOGV("Routing FM audio to No microphone Wired Headset (%d,%x)\n", mMode, outputDevices);
                    new_snd_device = SND_DEVICE_FM_HEADSET;
					new_post_proc_feature_mask = (ADRC_ENABLE | EQ_ENABLE | RX_IIR_ENABLE | MBADRC_ENABLE);
                } else
#endif                
                {
                    LOGV("Routing audio to No microphone Wired Headset (%d,%x)\n", mMode, outputDevices);
    ...
    if (mDualMicEnabled && mMode == AudioSystem::MODE_IN_CALL) {
        if (new_snd_device == SND_DEVICE_HANDSET) {
            LOGV("Routing audio to handset with DualMike enabled\n");
            new_snd_device = SND_DEVICE_IN_S_SADC_OUT_HANDSET;
        } else if (new_snd_device == SND_DEVICE_HEADSET) {
            LOGV("Routing audio to headset with DualMike enabled\n");
            new_snd_device = SND_DEVICE_FM_HEADSET;
        } else if (new_snd_device == SND_DEVICE_SPEAKER) {
            LOGV("Routing audio to speakerphone with DualMike enabled\n");
        ...
        AudioSystem::audio_in_acoustics acoustic_flags)
{
    ...
    mBufferSize = config.buffer_size;
      ...
      if (mDevices == AudioSystem::DEVICE_IN_VOICE_CALL)
      {
        ...
        LOGV("Recording Source: Mic/Headset");
        gcfg.rec_type = RPC_VOC_REC_REVERSE;
      }
    ...
    if (audpp_filter_inited)
    {
        ...
        if (enable_preproc_mask & AGC_ENABLE) {
            /* Setting AGC Params */
            LOGV("AGC Filter Param1= %02x.", tx_agc_cfg[audpre_index].cmd_id);
            LOGV("AGC Filter Param2= %02x.", tx_agc_cfg[audpre_index].tx_agc_param_mask);
            LOGV("AGC Filter Param3= %02x.", tx_agc_cfg[audpre_index].tx_agc_enable_flag);
            LOGV("AGC Filter Param4= %02x.", tx_agc_cfg[audpre_index].static_gain);
            LOGV("AGC Filter Param5= %02x.", tx_agc_cfg[audpre_index].adaptive_gain_flag);
            LOGV("AGC Filter Param6= %02x.", tx_agc_cfg[audpre_index].agc_params[0]);
            LOGV("AGC Filter Param7= %02x.", tx_agc_cfg[audpre_index].agc_params[18]);
            if ((enable_preproc_mask & AGC_ENABLE) &&
        ...
        if (enable_preproc_mask & NS_ENABLE) {
            /* Setting NS Params */
            LOGV("NS Filter Param1= %02x.", ns_cfg[audpre_index].cmd_id);
            LOGV("NS Filter Param2= %02x.", ns_cfg[audpre_index].ec_mode_new);
            LOGV("NS Filter Param3= %02x.", ns_cfg[audpre_index].dens_gamma_n);
            LOGV("NS Filter Param4= %02x.", ns_cfg[audpre_index].dens_nfe_block_size);
            LOGV("NS Filter Param5= %02x.", ns_cfg[audpre_index].dens_limit_ns);
            LOGV("NS Filter Param6= %02x.", ns_cfg[audpre_index].dens_limit_ns_d);
            LOGV("NS Filter Param7= %02x.", ns_cfg[audpre_index].wb_gamma_e);
            LOGV("NS Filter Param8= %02x.", ns_cfg[audpre_index].wb_gamma_n);
            if ((enable_preproc_mask & NS_ENABLE) &&
        ...
        if (enable_preproc_mask & TX_IIR_ENABLE) {
            ...
            LOGV("TX_IIR Filter Param1= %02x.", tx_iir_cfg[audpre_index].cmd_id);
            LOGV("TX_IIR Filter Param2= %02x.", tx_iir_cfg[audpre_index].active_flag);
            LOGV("TX_IIR Filter Param3= %02x.", tx_iir_cfg[audpre_index].num_bands);
            LOGV("TX_IIR Filter Param4= %02x.", tx_iir_cfg[audpre_index].iir_params[0]);
            LOGV("TX_IIR Filter Param5= %02x.", tx_iir_cfg[audpre_index].iir_params[1]);
            LOGV("TX_IIR Filter Param6 %02x.", tx_iir_cfg[audpre_index].iir_params[47]);
            if ((enable_preproc_mask & TX_IIR_ENABLE) &&
...
ssize_t AudioHardware::AudioStreamInMSM72xx::read( void* buffer, ssize_t bytes)
{
    ...
    while (count > 0) {
        ...
        {
         LOGV("Bytes Read = %d ,Buffer no longer sufficient",bytesRead);
         break;
        } else {
