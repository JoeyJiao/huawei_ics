...
//#define LOG_NDEBUG 0
#define LOG_TAG "AudioHardwareMSM7625"
#include <utils/Log.h>
   ...
   if (get_audpp_filter() == 0) {
    ...
    if (m7xsnddriverfd >= 0) {
        ...
        if (rc >= 0) {
            ...
            mInit = true;
            //LOGI("constructed (%d SND endpoints)", rc);
...
status_t AudioHardware::setParameters(const String8& keyValuePairs)
{
    ...
    //LOGI("setParameters() %s", keyValuePairs.string());//routing=2
    ...
    if (param.get(key, value) == NO_ERROR) {
        ...
        if (mBluetoothId == 0) {
            //LOGI("Using default acoustic parameters (%s not in acoustic database)", value.string());
            doRouting(NULL);
        }
    }
...
String8 AudioHardware::getParameters(const String8& keys)
{
    ...
    //LOGI("AudioHardware::getParameters() %s", param.toString().string());
...
int check_and_set_audpp_parameters(char *buf, int size)
{
    ...
    if ((buf[0] == 'A') && ((buf[1] == '1') || (buf[1] == '2') || (buf[1] == '3'))) {
     ...
     /* This is the MB_ADRC record we are looking for.  Tokenize it */
        ...
        //LOGI("MBADRC flag = %02x.", mbadrc_flag[device_id]);
     ...
     //Pre-Processing Features TX_IIR,NS,AGC
        ...
        //LOGI("TX IIR flag = %02x.", txiir_flag[device_id]);
        ...
        //LOGI("AGC flag = %02x.", agc_flag[device_id]);
        ...
        //LOGI("NS flag = %02x.", ns_flag[device_id]);
...
/*
    ...
    if(mbadrc_filter_exists[device_id] && state)
    {
        ...
        {
            //LOGI("MBADRC Enabled %d", post_proc_feature_mask);
    ...
    {
        post_proc_feature_mask &= MBADRC_DISABLE;
        //LOGI("ADRC Enabled %d", post_proc_feature_mask);
...
/*There is fucking thing since R3 after recompile android src, 
    ...
    //if(isInCall())
	//		mMicMute=false;
    //LOGI("Inside device=%d,isInCall=%s,SND_DEVICE_SPEAKER=%d",device,isInCall()?"true":"false",SND_DEVICE_SPEAKER);
...
status_t AudioHardware::AudioStreamOutMSM72xx::setParameters(const String8& keyValuePairs)
{
    ...
    status_t status = NO_ERROR;
    int device;
    //LOGI("AudioStreamOutMSM72xx::setParameters() %s", keyValuePairs.string());

    if (param.getInt(key, device) == NO_ERROR) {
        mDevices = device;
        //LOGI("set output routing %x", mDevices);
        ...
        AudioSystem::audio_in_acoustics acoustic_flags)
{
    ...
    mBufferSize = config.buffer_size;
      ...
      //LOGI("The Config buffer size is %d", config.buffer_size);
      //LOGI("The Config buffer count is %d", config.buffer_count);
      //LOGI("The Config Channel count is %d", config.channel_count);
      //LOGI("The Config Sample rate is %d", config.sample_rate);
      ...
      //LOGI("After set rec_type = 0x%8x\n",gcfg.rec_type);
      //LOGI("After set rec_interval_ms = 0x%8x\n",gcfg.rec_interval_ms);
      //LOGI("After set auto_stop_ms = 0x%8x\n",gcfg.auto_stop_ms);
      //LOGI("After set capability = 0x%8x\n",gcfg.capability);
      //LOGI("After set max_rate = 0x%8x\n",gcfg.max_rate);
      //LOGI("After set min_rate = 0x%8x\n",gcfg.min_rate);
      //LOGI("After set frame_format = 0x%8x\n",gcfg.frame_format);
      //LOGI("After set dtx_enable = 0x%8x\n",gcfg.dtx_enable);
      //LOGI("After set data_req_ms = 0x%8x\n",gcfg.data_req_ms);
    }
...
AudioHardware::AudioStreamInMSM72xx::~AudioStreamInMSM72xx()
{
    //LOGI("AudioStreamInMSM72xx destructor");
    standby();
}

ssize_t AudioHardware::AudioStreamInMSM72xx::read( void* buffer, ssize_t bytes)
{
    //LOGI("AudioStreamInMSM72xx::read(%p, %ld)", buffer, bytes);
    ...
    while (count > 0) {
        ...
        if (bytesRead > 0) {
            //LOGI("Number of Bytes read = %d", bytesRead);
            count -= bytesRead;
            p += bytesRead;
            bytes += bytesRead;
            //LOGI("Total Number of Bytes read = %d", bytes);
...
status_t AudioHardware::AudioStreamInMSM72xx::setParameters(const String8& keyValuePairs)
{
    ...
    status_t status = NO_ERROR;
    int device;
    //LOGI("AudioStreamInMSM72xx::setParameters() %s", keyValuePairs.string());
